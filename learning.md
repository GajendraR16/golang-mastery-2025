...
Task API implementation-specific notes have been moved to `task-api-notes.md` to keep this file focused on core Go concepts and reusable patterns.

## Concurrency, Context, and Production Patterns

### Concurrency Basics

- Concurrency in Go is about structuring work independently; parallelism is running work at the same time.
- Goroutines are lightweight and cheap, but not free. Always define who owns lifecycle and cancellation.
- Shared mutable state needs protection (`sync.Mutex`, `sync.RWMutex`, atomics) or a message-passing design.

### Channels

- Channels are typed conduits for communication between goroutines.
- Use unbuffered channels for synchronization handoff, buffered channels for bounded queues.
- Closing rule: only the sender should close a channel, and close it exactly once.
- Use `select` with `ctx.Done()` to avoid goroutine leaks.
...
- **Fan-out**: distribute work from one input channel to multiple workers.
- **Fan-in**: merge multiple worker output channels into one result channel.
- Use `sync.WaitGroup` to know when all workers are done before closing the merged output channel.

### Semaphore Pattern

- Use a buffered channel as a semaphore to cap concurrent operations (DB calls, HTTP requests, file IO).
...
This prevents resource spikes and helps maintain stable latency.

### Worker Pools

- Worker pool pattern = fixed number of workers reading from a `jobs` channel and writing to `results`.
- Benefits: bounded concurrency, predictable memory usage, easier backpressure handling.
- Always define queue size, worker count, cancellation behavior, and shutdown behavior.

### Context Usage

- Pass `ctx` from request entrypoint down to DB/external calls.
- Use `context.WithTimeout` or `context.WithDeadline` for bounded operations.
- Keep context values request-scoped and small (`request_id`, `user_id`, trace IDs).
- Never replace active request context with `context.Background()` in middle layers.

### Bug Diary: Context Wipe in Timeout Middleware

- Bug: in timeout middleware, using `context.Background()` to create timeout context detached the request from existing values/cancellation.
- Impact: `/tasks/{id}` calls lost upstream context data and broke expected propagation.
- Correct approach: derive timeout context from `r.Context()`.

```go
// Wrong: wipes request context chain
ctx, cancel := context.WithTimeout(context.Background(), timeout)

// Correct: preserves values + cancellation chain
ctx, cancel := context.WithTimeout(r.Context(), timeout)
```

### Graceful Shutdown

- On `SIGINT`/`SIGTERM`: stop accepting new work, cancel root context, wait for in-flight work with timeout, then close resources.
- `http.Server.Shutdown(ctx)` lets in-flight HTTP requests finish gracefully within a deadline.

### Structured Logging

- Prefer structured logs (`logger.Info("task fetched", "task_id", id, "status", status)`) over plain text.
- Use stable keys (`request_id`, `task_id`, `user_id`, `duration_ms`, `error`) for search, dashboards, and alerts.
- Add context metadata to logs for cross-layer traceability.
- Avoid secrets/full payloads in logs.
